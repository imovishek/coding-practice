		...
		for(int i = 0; i<n; i++)
		{
			for(int j = i+1; j<n; j++)
			{
				if(cmp(dist(ara[i].c, ara[j].c) - (ara[i].r + ara[j].r)) < 0){
					...
					for(int k = 0; k<4; k++)
					{
						if(cmp(cross(ara[i].c-ara[j].c, rec[k].first-ara[j].c)) < 0) lft++;
						else if(cmp(cross(ara[i].c-ara[j].c, rec[k].first-ara[j].c)) > 0) rght++;
						else cnt++;
					}
					if(lft == 4 || lft+cnt==4 || rght == 4 || rght+cnt==4){
